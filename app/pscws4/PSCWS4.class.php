<?php
namespace my_calendar_server_reborn\app\pscws4;
...
{	
	...
	// FOR PHP5
	function __construct() { $this->_PSCWS4(); }
	...
	function _msegment($end, $zlen)
	{
		...
		for ($i = 0; $start < $end; $i++)
		{
			...
			if ($clen == 1)
			{
				while ($start++ < $end)
				{
					$cx = isset($txt[$start]) ? ord($txt[$start]) : null;
					if (isset($cx) && $this->_ztab[$cx] > 1) break;
					$clen++;				
				}
