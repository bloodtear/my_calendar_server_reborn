...
class Db_tempuser extends fdb\Database_table {
    ...
    public function add($openid, $uid, $nickname, $avatar, $create_time, $last_login, $token, $status, $calendar_session) {
        return $this->insert(array("openid" => $openid, "uid" => 0, "nickname" => $nickname, "avatar" => $avatar, "create_time" => time(), "last_login" => time(), "token" => $token, "status" => $status, "calendar_session" => $calendar_session));
    }

    public function modify($id, $openid, $uid, $nickname, $avatar, $create_time, $last_login, $token, $status, $calendar_session) {
        $id = (int)$id;
        return $this->update(array("openid" => $openid, "uid" => $uid, "nickname" => $nickname, "avatar" => $avatar, "create_time" => $create_time, "last_login" => time(), "token" => $token, "status" => $status, "calendar_session" => $calendar_session), "id = $id");
    }
